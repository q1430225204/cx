...
class sell {
	...
	function update($itemid, $item = array(), $content = '') {
		...
		$this->db->query("REPLACE INTO {$this->table}_search (itemid,catid,areaid,status,content,sorttime) VALUES ($itemid,'$item[catid]','$item[areaid]','$item[status]','$keyword','$sorttime')");
	}
	...
	function recycle($itemid) {
		if(is_array($itemid)) {
			...
			$this->db->query("UPDATE {$this->table}_search SET status=0 WHERE itemid=$itemid");
	...
	function restore($itemid) {
		...
		if(is_array($itemid)) {
			...
			$this->db->query("UPDATE {$this->table}_search SET status=3 WHERE itemid=$itemid");
	...
	function delete($itemid, $all = true) {
		...
		if(is_array($itemid)) {
			...
			if($all) {
				...
				$this->db->query("DELETE FROM {$this->table}_search WHERE itemid=$itemid");
				$content_table = content_table($this->moduleid, $this->itemid, $this->split, $this->table_data);
	...
	function check($itemid) {
		...
		if(is_array($itemid)) {
			...
			$this->db->query("UPDATE {$this->table}_search SET status=3,sorttime='$sorttime' WHERE itemid=$itemid");
			$this->tohtml($itemid);
			return true;
		}
	}
	...
	function reject($itemid) {
		...
		if(is_array($itemid)) {
			...
			$this->db->query("UPDATE {$this->table}_search SET status=1 WHERE itemid=$itemid");
			return true;
		}
	}
	...
	function expire($condition = '') {
		...
		while($r = $this->db->fetch_array($result)) {
			...
			$this->db->query("UPDATE {$this->table}_search SET status=4 WHERE itemid=$itemid");
		}		
	}
	...
	function refresh($itemid) {
		...
		if(is_array($itemid)) {
			...
			$sorttime = $this->get_sorttime($DT_TIME, $item['vip']);
			$this->db->query("UPDATE {$this->table}_search SET sorttime='$sorttime' WHERE itemid=$itemid");
		}
	}
