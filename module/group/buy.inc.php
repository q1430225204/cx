...
if($item && $item['status'] > 2) {
	...
	dalert(lang('message->item_not_exists'), $MOD['linkurl']);
}
...
if($submit) {
	...
	$db->query("INSERT INTO {$DT_PRE}group_order (gid,buyer,seller,title,linkurl,thumb,price,number,amount,logistic,addtime,updatetime,note,password, buyer_postcode,buyer_address,buyer_name,buyer_phone,buyer_mobile,buyer_receive) VALUES ('$itemid','$_username','$item[username]','$title','$linkurl','$item[thumb]','$item[price]','$number','$amount','$item[logistic]','$DT_TIME','$DT_TIME','$note','$password','$buyer_postcode','$buyer_address','$buyer_name','$buyer_phone','$buyer_mobile','$buyer_receive')");
	//send sms
