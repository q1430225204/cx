...
if($action=='appregister'){
	...
	//if($msg) echo $msg;
	//if($_SESSION['regemail'] != md5(md5($post['email'].DT_KEY.$DT_IP))) echo $L['check_sign'];
	$RG = array();
	...
	if($do->add($post)) {
		...
		$reload_captcha = $MOD['captcha_register'] ? reload_captcha() : '';
		$reload_question = $MOD['question_register'] ? reload_question() : '';
		//echo $reload_captcha.$reload_question;exit;
		//dalert('1');
		//echo $do->errmsg;exit;
