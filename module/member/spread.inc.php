...
if($action == 'add') {
	...
	if($kw && in_array($mid, array(4, 5, 6))) {
		...
		$r = $db->get_one("SELECT * FROM {$DT_PRE}spread WHERE username='$_username' AND mid=$mid AND word='$kw' AND fromtime>=$next_time");
		...
		if($r['num'] > $spread_max) message(lang($L['spread_msg_over'], array($kw)), $EXT['spread_url']);
		...
		if($submit) {
			...
			if($currency == 'money') {
				...
				money_record($_username, -$amount, $L['in_site'], 'system', $MODULE[$mid]['name'].$L['spread_title'], $kw.'('.$L['spread_infoid'].$buy_tid.')');
			} else {
				credit_add($_username, -$amount);
				credit_record($_username, -$amount, 'system', $MODULE[$mid]['name'].$L['spread_title'], $kw.'(ID:'.$buy_tid.')');
			}
