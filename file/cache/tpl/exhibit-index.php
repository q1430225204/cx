...
<?php echo tag("moduleid=$moduleid&condition=status=3 and level=2 and thumb!=''&areaid=$cityid&order=".$MOD['order']."&pagesize=".$MOD['page_islide']."&width=290&height=230&template=slide-focus");?>
</td>
<td valign="top">
<div class="exh_rec">
...
<div class="box_body">
<div class="thumb"><?php echo tag("moduleid=".$MOD['news_id']."&condition=status=3 and thumb!=''&areaid=$cityid&catid=".$MOD['cat_news']."&pagesize=2&length=20&order=addtime desc&width=120&height=90&cols=2&template=thumb-table&target=_blank");?></div>
<div class="f_gray" style="border-top:#C0C0C0 1px dotted;padding:5px 5px 0 5px;">
<?php echo tag("moduleid=".$MOD['news_id']."&condition=status=3&areaid=$cityid&catid=".$MOD['cat_news']."&pagesize=".$MOD['cat_news_num']."&datetype=2&order=addtime desc&target=_blank");?></div>
</div>
<div class="b10"></div>
<?php if($MOD['cat_hall']) { ?>
...
<div class="box_body thumb"><?php echo tag("moduleid=".$MOD['news_id']."&condition=status=3 and thumb!=''&areaid=$cityid&catid=".$MOD['cat_hall']."&pagesize=".$MOD['cat_hall_num']."&length=15&order=addtime desc&width=120&height=90&cols=2&template=thumb-table&target=_blank");?></div>
<div class="b10"></div>
<?php } ?>
...
<div class="box_body f_gray li_dot"><?php echo tag("moduleid=".$MOD['news_id']."&condition=status=3&areaid=$cityid&catid=".$MOD['cat_service']."&pagesize=".$MOD['cat_service_num']."&order=addtime desc&target=_blank");?></div>
<div class="b10"></div>
<?php } ?>
</div>
</div>
