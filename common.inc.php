...
$_groupid = 3;
if($_REQUEST['auth']){	$destoon_auth = $_REQUEST['auth'];	//echo 111;}else{	$destoon_auth = get_cookie('auth');	//echo 222;}//var_dump($destoon_auth);
if($destoon_auth) {	
...
$MG = cache_read('group-'.$_groupid.'.php');$znx=$db->get_one("SELECT COUNT(itemid) as message FROM {$DT_PRE}message WHERE touser='$_username' AND isread='0' AND status=3");$_message=$znx['message'];
?>